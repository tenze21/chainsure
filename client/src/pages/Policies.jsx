...
const POLICIES = [
  {
    id: 'travel', name: 'Travel Insurance', policyNumber: 'TRV-2024-001042', status: 'Active',
    coverage: 'Nu. 10,00,000', premium: 'Nu. 5000/ year', expires: 'Dec 15, 2026',
    startDate: 'Dec 15, 2024', deductible: 'Nu. 2500', extraLabel: 'Covered Destination', extraValue: 'Thimphu, Bhutan',
  ...
  {
    id: 'motor', name: 'Motor Insurance', policyNumber: 'MTR-2024-001041', status: 'Active',
    coverage: 'Nu. 25,00,000', premium: 'Nu. 5000/ year', expires: 'Mar 01, 2027',
    startDate: 'Dec 15, 2024', deductible: 'Nu. 2500', extraLabel: 'Vehicle Details', extraValue: 'Toyota Fortuner - BP-1-A-1234',
  ...
  {
    id: 'life', name: 'Life Insurance', policyNumber: 'LIF-2024-001040', status: 'Active',
    coverage: 'Nu. 50,00,000', premium: 'Nu. 5000/ year', expires: 'Jan 10, 2046',
    startDate: 'Dec 15, 2024', deductible: 'Nu. 2500', extraLabel: 'Beneficiary', extraValue: 'Karma Dorji (Spouse)',
        ...
        <main className="policies">
          ...
          <div className="policies__body">
            <div className="policy-list">
              {POLICIES.map((policy) => (
                <div key={policy.id} className={`policy-card ${selected?.id === policy.id ? 'policy-card--selected' : ''}`} onClick={() => setSelected(policy)}>
                  ...
                  <div className="policy-card__stats">
                    ...
                    <div className="policy-stat">
                      <span className="policy-stat__label">{selected?.id === policy.id ? 'Expires' : 'Premium'}</span>
                      <span className="policy-stat__value">{selected?.id === policy.id ? policy.expires : policy.premium}</span>
                    </div>
                  </div>
                </div>
              ))}
                ...
                <div className="policy-detail__content">
                  <div className="policy-detail__heading">
                    ...
                    <div>
                      ...
                      <span className="badge badge--active" style={{ marginTop: 4, display: 'inline-flex' }}><span className="badge__dot" />{selected.status}</span>
                    </div>
                  </div>
                  <div className="policy-detail__grid">
                    <div className="detail-field"><span className="detail-field__label">Policy Number</span><span className="detail-field__value">{selected.policyNumber}</span></div>
                    <div className="detail-field"><span className="detail-field__label">Premium</span><span className="detail-field__value">{selected.premium}</span></div>
                    <div className="detail-field"><span className="detail-field__label">Coverage Amount</span><span className="detail-field__value">{selected.coverage}</span></div>
                    <div className="detail-field"><span className="detail-field__label">Start Date</span><span className="detail-field__value">{selected.startDate}</span></div>
                    <div className="detail-field detail-field--full"><span className="detail-field__label">Deductible</span><span className="detail-field__value">{selected.deductible}</span></div>
                    <div className="detail-field detail-field--full"><span className="detail-field__label">{selected.extraLabel}</span><span className="detail-field__value">{selected.extraValue}</span></div>
                  </div>
                  <div className="policy-detail__actions">
                    <button className="detail-btn detail-btn--outline" onClick={() => generatePolicyPDF(selected)}>
                    ...
                    <button className="detail-btn detail-btn--ghost">File a Claim</button>
                  </div>
                </div>
              )}
            </div>
