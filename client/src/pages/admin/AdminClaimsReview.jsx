...
const initialClaims = [
  ...
  },
  {
    ...
    status: 'pending',
    description: 'Vehicle stolen overnight. Police report submitted.',
  },
  {
    id: 'CLM-020',
    policyholder: 'Sonam Peki',
    email: 'peki@example.com',
    policy: 'Life Insurance #1040',
    type: 'Medical',
    amount: 'Nu. 50,000',
    priority: 'Not assigned',
    status: 'approved',
    description: 'Routine claim for outpatient procedures.',
  },
]
...
export default function AdminClaimsReview() {
  ...
  const filteredClaims = useMemo(() => {
    if (activeTab === 'pending') return claims.filter((claim) => claim.status === 'pending')
  ...
  const handleSave = () => {
    updateClaim({ priority: reviewData.priority, notes: reviewData.notes })
    closeReview()
  }

  const handleDecision = (status) => {
    updateClaim({ status, priority: reviewData.priority, notes: reviewData.notes })
    closeReview()
  }
  ...
  return (
    <div>
      ...
      <div className="mb-4 p-3 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-600">
        Claims endpoints are not available in the backend yet. Showing sample data until APIs are added.
      </div>

      <div className="flex items-center gap-4 mb-4 text-sm">
        <button
          onClick={() => setActiveTab('pending')}
          className={`px-3 py-1 rounded-full ${activeTab === 'pending' ? 'bg-[#0f1729] text-white' : 'text-gray-500'}`}
        >
          Pending claims
        </button>
        <button
          onClick={() => setActiveTab('covered')}
          className={`px-3 py-1 rounded-full ${activeTab === 'covered' ? 'bg-[#0f1729] text-white' : 'text-gray-500'}`}
        >
          Covered claims
        </button>
      </div>
          ...
          <div className="bg-white rounded-xl w-full max-w-xl p-6">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
              ...
              <div className="bg-gray-50 rounded-lg p-3">
                ...
                <div className="font-medium text-gray-900">{selectedClaim.amount}</div>
              </div>
              <div className="bg-gray-50 rounded-lg p-3">
                <div className="text-xs text-gray-500">Status</div>
                <div className="font-medium text-gray-900 capitalize">{selectedClaim.status}</div>
              </div>
            </div>
            ...
            <button
              type="button"
              className="w-full py-2.5 bg-[#0f1729] text-white rounded-lg text-sm font-medium mb-4"
              onClick={() => alert('Download policy document')}
            >
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              <button
                ...
                className="px-4 py-2 text-sm bg-red-50 text-red-600 border border-red-200 rounded-lg"
              >
                Deny Claim
              </button>
              <button
                onClick={() => handleDecision('approved')}
                className="px-4 py-2 text-sm bg-green-50 text-green-700 border border-green-200 rounded-lg"
              >
                Approve Claim
              </button>
            </div>
