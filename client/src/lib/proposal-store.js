...
export function saveTrackedProposal(proposal) {
  ...
  if (Object.prototype.hasOwnProperty.call(proposal, 'policyId')) {
    nextProposal.policyId = normalizeOptionalText(proposal.policyId)
  }

  if (Object.prototype.hasOwnProperty.call(proposal, 'policyStatus')) {
    nextProposal.policyStatus = normalizeOptionalText(proposal.policyStatus)
  }
...
export function attachTrackedPolicyToProposal({ proposalId, policyId, policyStatus }) {
...
export function updateTrackedPolicyStatusByPolicyId({ policyId, policyStatus }) {
  if (!policyId || !policyStatus) {
    return null
  }

  const proposals = parseStoredProposals()
  const matchingProposal = proposals.find((item) => item.policyId === policyId)
